...
class App extends Component {
  constructor( props ) {
    ...
    this.ships = [
      { id: 0, x: 50, y: 50, type: 1, color: 'Red' },
      { id: 1, x: 100, y: 50, type: 1, color: 'Red' },
      { id: 2, x: 150, y: 50, type: 1, color: 'Red' }
    ];
    this.lasers = [];
  ...
  render() {
    let shipObjects = this.ships.map( (ship) => {
      ...
      return <img src={'/images/enemy' + ship.color + ship.type + '.png'} alt='Enemy' style={enemyShipStyle} className='ship' key={'enemy'+ship.id} />
    });
    let laserObjects = this.lasers.map( (laser, index) => {
      ...
      return <img src='/images/laserRed01.png' alt='Laser' style={laserStyle} className='laser' key={'laser' + index} />
    })
    let playerShipStyle = {
      left: this.state.x,
      top: this.state.y
